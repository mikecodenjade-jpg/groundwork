...
type FocusArea = 'full' | 'upper' | 'lower' | 'core';

const TRADES: Record<string, string> = {
  'General Laborer': 'general_laborer', 'Carpenter': 'carpenter', 'Electrician': 'electrician',
  'Plumber': 'plumber', 'Ironworker': 'ironworker', 'Mason/Bricklayer': 'mason_bricklayer',
  'Roofer': 'roofer', 'Painter': 'painter', 'HVAC Technician': 'hvac_technician',
  'Heavy Equipment Operator': 'heavy_equipment_operator', 'Welder': 'welder',
  'Pipefitter': 'pipefitter', 'Superintendent/Foreman': 'superintendent_foreman'
};
...
export default function TrainPage() {
  ...
  const [equipment, setEquipment] = useState(['barbell','dumbbell','kettlebell','bodyweight']);
  ...
  const toKg = (v: string) => useLbs && v ? Math.round(parseFloat(v) / 2.205) : parseFloat(v);
  const expYears = (exp: string) => exp === 'beginner' ? 0.5 : exp === 'intermediate' ? 2 : 5;
  ...
  const loadWorkout = useCallback(async (uid: string) => {
    ...
    try {
      const [fRes, wRes] = await Promise.all([
        ...
        supabase.rpc('recommend_workout_v2', { p_user_id: uid, p_focus_area: focus, p_equipment: equipment, p_max_minutes: 45 })
      ]);
      if (fRes.error) console.error('fatigue error:', fRes.error);
      if (wRes.error) { console.error('workout error:', wRes.error); setError(wRes.error.message); }
  ...
  const handleOnboard = async () => {
    ...
    try {
      const params: Record<string, any> = {
        p_user_id: userId,
        p_body_weight_kg: weightKg,
        p_height_cm: heightCm,
        p_age: age,
        p_sex: sex,
        p_training_years: expYears(experience),
        p_training_days_per_week: 3,
        p_trade_slug: TRADES[trade] || 'general_laborer',
        p_equipment: equipment,
        p_training_location: 'mixed',
        p_max_minutes: 45,
      ...
      const { error: onboardErr } = await supabase.rpc('onboard_user', params);
      if (onboardErr) { setError(onboardErr.message); setLoading(false); return; }
  ...
  const logShift = async (hours: number) => {
    await supabase.rpc('ingest_activity', { p_user_id: userId, p_activity_type: 'occupational', p_activity_name: trade, p_duration_minutes: hours * 60, p_intensity: 0.7 });
    await loadWorkout(userId);
  };
  ...
  if (step === 'onboarding') {
    const steps = [
      ...
      <div key="s2">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 24 }}>{Object.keys(TRADES).map(t =>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>{EQUIPMENT.map(e =>
          <button key={e} onClick={() => setEquipment(p => p.includes(e)?p.filter(x=>x!==e):[...p,e])} style={{ padding: '8px 14px', background: equipment.includes(e)?'#C45B28':'#222', border: '1px solid #444', borderRadius: 20, color: '#fff', fontSize: 13, cursor: 'pointer' }}>{e.replace(/_/g,' ')}</button>
        )}</div>
      </div>,
      <div key="s3">
      ...
      <div style={{ minHeight: '100vh', background: '#111', color: '#fff' }}>
        <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px 100px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
            ...
            <button onClick={() => onboardStep < 3 ? setOnboardStep(s => s+1) : handleOnboard()} disabled={loading} style={{ flex:2, padding:14, background:'#C45B28', border:'none', borderRadius:8, color:'#fff', fontSize:16, fontWeight:700, cursor:'pointer', opacity: loading?0.6:1 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#111', color: '#fff' }}>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px 100px' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>{(['full','upper','lower','core'] as FocusArea[]).map(f =>
          <button key={f} onClick={() => { setFocus(f); loadWorkout(userId); }} style={{ flex:1, padding:'8px 4px', background: focus===f?'#C45B28':'#222', border:'1px solid #444', borderRadius:8, color:'#fff', fontSize:12, cursor:'pointer', textTransform:'capitalize' as const }}>{f === 'full' ? 'Full Body' : f}</button>
        )}</div>
        ...
        {fatigue.length > 0 && (
          <div style={{ background: '#1a1a1a', borderRadius: 12, padding: 16, marginBottom: 20, border: '1px solid #333' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {fatigue.filter((f: any) => f.decayed_score > 5).sort((a: any, b: any) => b.decayed_score - a.decayed_score).slice(0,8).map((f: any) =>
                <div key={f.muscle_slug} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: '#ccc', textTransform: 'capitalize' as const }}>{(f.muscle_slug || '').replace(/_/g,' ')}</div>
                    <div style={{ height: 4, background: '#333', borderRadius: 2, marginTop: 4 }}><div style={{ height: '100%', width: Math.min(f.decayed_score,100)+'%', background: fatColor(f.decayed_score), borderRadius: 2 }} /></div>
                  </div>
                  <span style={{ fontSize: 10, color: fatColor(f.decayed_score), fontWeight: 700 }}>{fatLabel(f.decayed_score)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div>
            ...
            {workout.session && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                <div style={{ flex:1, background:'#1a1a1a', borderRadius:8, padding:10, textAlign:'center', border:'1px solid #333' }}>
                  <div style={{ fontSize:20, fontWeight:700, color:'#C45B28' }}>{workout.session.main_exercises || 0}</div><div style={{ fontSize:11, color:'#999' }}>Exercises</div>
                </div>
                <div style={{ flex:1, background:'#1a1a1a', borderRadius:8, padding:10, textAlign:'center', border:'1px solid #333' }}>
                  <div style={{ fontSize:16, fontWeight:700, color:'#C45B28', textTransform:'capitalize' as const }}>{workout.session.phase || 'Accumulation'}</div><div style={{ fontSize:11, color:'#999' }}>Phase</div>
                </div>
                <div style={{ flex:1, background:'#1a1a1a', borderRadius:8, padding:10, textAlign:'center', border:'1px solid #333' }}>
                  <div style={{ fontSize:20, fontWeight:700, color:'#C45B28' }}>~{workout.session.estimated_minutes || 0}</div><div style={{ fontSize:11, color:'#999' }}>Minutes</div>
                </div>
              </div>
            )}
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {workout.warmup.map((ex: any, i: number) =>
                  <div key={i} style={{ background:'#1a1a1a', borderRadius:8, padding:'10px 14px', marginBottom:6, border:'1px solid #333', display:'flex', justifyContent:'space-between' }}>
                    <span style={{ fontSize:14, color:'#ccc' }}>{ex.name}</span>
                    <span style={{ fontSize:12, color:'#666' }}>{ex.duration_seconds}s</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {workout.workout?.map((ex: any, i: number) => (
                <div key={i} style={{ background:'#1a1a1a', borderRadius:12, padding:14, marginBottom:8, border:'1px solid #333' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    <div>
                      <div style={{ fontSize:16, fontWeight:600, color:'#fff' }}>{ex.name}</div>
                      <div style={{ fontSize:12, color:'#666' }}>{ex.movement_pattern} {ex.phase && `• ${ex.phase}`}</div>
                    </div>
                    <div style={{ fontSize:12, color:'#C45B28', fontWeight:700 }}>Score: {ex.composite_score}</div>
                  </div>
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap' as const }}>
                    <div style={{ background:'#222', borderRadius:8, padding:'6px 12px' }}>
                      <div style={{ fontSize:11, color:'#999' }}>Sets x Reps</div>
                      <div style={{ fontSize:14, color:'#fff', fontWeight:600 }}>{ex.prescribed_sets} x {ex.prescribed_reps}</div>
                    </div>
                    {ex.prescribed_weight_kg && ex.prescribed_weight_kg !== 'null' && (
                      <div style={{ background:'#222', borderRadius:8, padding:'6px 12px' }}>
                        <div style={{ fontSize:11, color:'#999' }}>Weight</div>
                        <div style={{ fontSize:14, color:'#fff', fontWeight:600 }}>{toDisplay(parseFloat(ex.prescribed_weight_kg))} {unitLabel}</div>
                      </div>
                    )}
                    <div style={{ background:'#222', borderRadius:8, padding:'6px 12px' }}>
                      <div style={{ fontSize:11, color:'#999' }}>RPE</div>
                      <div style={{ fontSize:14, color:'#fff', fontWeight:600 }}>{ex.rpe_target || 7}</div>
                    </div>
                  </div>
                  {ex.progression_note && ex.progression_note !== 'null' && <div style={{ fontSize:11, color:'#999', marginTop:6, fontStyle:'italic' }}>{ex.progression_note}</div>}
                </div>
              ))}
            </div>
